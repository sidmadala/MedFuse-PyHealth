...
class FusionTrainer(Trainer):
    ...
    def validate(self, dl):
        ...
        i = 1  # NOTE: Adding this to prevent UnboundLocalError: local variable 'i' referenced before assignment

        # print(f"dl length: {len(dl)}")
        with torch.no_grad():
            for i_, (x, img, y_ehr, y_cxr, seq_lengths, pairs) in enumerate(dl):
                print("Inside validation loop")
                i = i_ # NOTE: Hoisting scope of i_ to be accessible outside of the loop
        ...
        self.scheduler.step(epoch_loss/(len(self.val_dl) + 0.0000001))  # NOTE: Adding small value to avoid division by zero

        print(f"val [{self.epoch:04d} / {self.args.epochs+1:04d}]")
        print(f"validation loss: \t{epoch_loss/(i+1):0.5f} \t{epoch_loss_align/(i+1):0.5f}")
        ...
        self.epochs_stats['loss val'].append(epoch_loss/(i+1))
        self.epochs_stats['loss align val'].append(epoch_loss_align/(i+1))
    ...
    def eval_age(self):
        ...
        patiens = pd.read_csv('/srv/local/data/MIMIC-IV/2.0/hosp/patients.csv')
