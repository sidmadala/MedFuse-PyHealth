...
def transform_ethnicity(ethnicity_series):
    ...
    return {'Race': ethnicity_series.fillna('').apply(lambda s: e_map[s] if s in e_map else e_map['OTHER'])}  # NOTE: Changed ethnicity to race
...
def assemble_episodic_data(stays, diagnoses):
    ...
    data.update(transform_ethnicity(stays.race))  # NOTE: Changed ethnicity to race
    data['Height'] = np.nan
    data['Weight'] = np.nan
    data = DataFrame(data).set_index('Icustay')
    data = data[['Race', 'Gender', 'Age', 'Height', 'Weight', 'Length of Stay', 'Mortality']]  # NOTE: Changed ethnicity to race
...
def clean_fio2(df):
    ...
    '''
    is_str = np.array(map(lambda x: type(x) == str, list(df.value)), dtype=np.bool_)  # Changed np.bool to np.bool_
